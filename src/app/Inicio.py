...
def run_app() -> None:
    ...
    st.markdown(
        """
    <style>
    .stApp { background: linear-gradient(180deg, #eef2f7 0%, #e6edf5 100%); }
    ...
    with header_col:
        ...
        <div class="brand-wrap">
            ...
            Plataforma para invertir en ganado con compra, sostenimiento en finca y reparto de utilidades.
            </div>
        </div>
        """,
            unsafe_allow_html=True,
    ...
    seccion = st.sidebar.radio("Ir a", ["Inicio", "¿Qué es LaCow?", "Fincas disponibles", "Simulador financiero"])
    ...
    if seccion == "Inicio":
            ...
            with st.form("form_registro"):
                ...
                if crear_cuenta:
                    if not all([nombre, correo, telefono, clave]) or not aceptar:
                        ...
                        st.success("¡Registro exitoso! Tu cuenta quedó creada en modo demostración.")
        ...
        st.markdown(
            """
            <div class="card">
            <h4>Modelo en campaña</h4>
            <p>El inversionista compra ganado, el ganadero aliado lo sostiene y al vender se reparten utilidades según el acuerdo (60/40 o 50/50).</p>
            <ul>
              <li>Compra de ganado por lote o por cabeza.</li>
              <li>Sostenimiento por ganadero aliado en finca.</li>
              <li>Seguimiento de peso, costos y rentabilidad.</li>
              <li>Liquidación y reparto de utilidades.</li>
            </ul>
            </div>
            """,
            unsafe_allow_html=True,
        ...
        for idx, finca in enumerate(fincas):
            ...
            st.write(f"🐄 Tipo: {finca['tipo']}")
            st.write(f"✅ Ganado disponible: {finca['ganado']} cabezas")
            st.write(f"💰 Precio estimado: {finca['precio_estimado']}")

            mensaje = quote(f"Hola LaCow, estoy interesado en {finca['nombre']}.")
            asunto = quote(f"Interés en {finca['nombre']}")
            cuerpo = quote(f"Quiero información de {finca['nombre']} en {finca['ubicacion']}.")
        ...
        with tab2:
            ...
            if ruta_historico.exists():
                st.dataframe(pd.read_excel(ruta_historico))
